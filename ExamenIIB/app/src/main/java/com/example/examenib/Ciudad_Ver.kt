...
class Ciudad_Ver : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       // findViewById<TextView>(R.id.txt_titulo_nombre_pais).setText(nombre_pais)
        ...
        if(codigo_ISO != null){
            db.paisApp!!.obtenerCiudadesPorPais(codigo_ISO) { ciudades ->
    ...
    override fun onContextItemSelected(item: MenuItem): Boolean {
        return when (item.itemId){
            R.id.mi_editar_ciudad ->{
                ...
                mostrarSnackbar(codigoISOPais.toString())
                val extras = Bundle()
                ...
                extras.putString("seguridad", seguridad)
                extras.putString("codigoISOPais", codigoISOPais.toString())
...
//                mostrarSnackbar(identificador)
    ...
    fun abrirDialogo(codigoCiudad: String, codigoISO: String): Boolean {
        ...
        builder.setPositiveButton(
            ...
            DialogInterface.OnClickListener { dialog, which ->
                ...
                db.paisApp?.eliminarCiudadPorCodEISO(codigoCiudad.toString(), codigoISO.toString())
                    ?.addOnSuccessListener {
                        mostrarSnackbar("Ciudad eliminada exitosamente")
                        cargarListaCiudades(codigoISO)
                        eliminacionExitosa = true
                    }
                    ?.addOnFailureListener { e ->
                        mostrarSnackbar("No se pudo eliminar esta ciudad")
        ...
        registerForActivityResult(
            ...
            if(result.resultCode == Activity.RESULT_OK){
                if(result.data != null){
                    ...
                    cargarListaCiudades("${data?.getStringExtra("codigoISOPais")}")
                    mostrarSnackbar("${data?.getStringExtra("codigoISOPais")}")
                }
            }
        }
    ...
    private fun cargarListaCiudades(codigoISO: String) {
        ...
        db.paisApp!!.obtenerCiudadesPorPais(codigoISO) { ciudades ->
            this.ciudades = ciudades
